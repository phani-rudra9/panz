      ...
      steps {
        script {
          ...
          if (env.ref == null || env.ref == '') {
            error "The ref variable is empty. Check your Generic Webhook Trigger configuration."
          } else if (env.ref == 'refs/heads/master') {
            echo "Success! The webhook triggered for the master branch."
          } else {
            error "The ref variable did not match refs/heads/master. Value: ${env.ref}"
          }
        }
      }
    }
